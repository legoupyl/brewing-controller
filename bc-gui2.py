...
class BK_power_controller_class(object):
    def __init__(self):
        print ("Initializing BK controller")
        self.mode="power"
        self.unit="%"
        self.running=False
        self.power =BK_setpoint
        self.freq=1
...
class BK_temp_controller_class(object):
    def __init__(self):
        self.mode="temp"
        self.unit="C"
        self.running =False
    ...
    def stop(self):
        print ("Stoping HLT temp controller" )
        self.running = False
    def run(self):
        self.running = True
        GPIO.setup(self.gpio_num, GPIO.OUT)
        print ("Running  HLT temp controller" )
        while self.running == True:
            T=1 / self.freq
            TimeOn= round (T * (self.power / 100),4)
            TimeOff = T - TimeOn
    ...
    def start(self):
        if self.running == False:
            print ("Starting BK Temp controller" )
            BK_heater_thread = threading.Thread(target=self.run)
            BK_heater_thread.daemon=True
            BK_heater_thread.start()
    ...
    def toggle(self):
        if self.running == False:
            print ("Toggle : Starting HLT temp controller" )
            self.start()
        else:
            self.stop()
            print ("Toggle : Stoping HLT temp controller" )
